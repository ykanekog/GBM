; $Id: //depot/idl/IDL_70/idldir/lib/utilities/xplot3d.pro#1 $
;
...
IF (keyword_set(kill)) THEN BEGIN
    ...
    if (wid ne 0l) then begin
		widget_control, wid, get_uvalue=pState
    	WIDGET_CONTROL, (*pState).wQuitButton, SEND_EVENT = {ID:0L,TOP:0L,HANDLER:0L}
    END
    
    RETURN
END

IF KEYWORD_SET(clear) THEN erase = 1 ELSE erase = 0
...
if (keyword_set(overplot)) then begin
    ...
    if (wid ne 0l) then begin
        ...
        IF erase THEN BEGIN
        	objs = oPolylineModel->get(/ALL)
        	oPolylineModel->Remove,/ALL
        	OBJ_DESTROY,objs
        ENDIF
...
; Added code to embed the draw widget into another hierarchy
    if keyword_set(base) then begin
		tlb = widget_base(base, $
            column=1 $
            )
        mbar = widget_base(tlb, /row, /frame)
    endif else if keyword_set(modal) then begin
        tlb = widget_base( $
            title='XPlot3D', $
            column=1, $
            tlb_size_events=1, $
